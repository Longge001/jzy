...
%%% Tweaking Erlang memory allocators and
%%% 调整erlang的内存分配器
%%% their behaviour is a very tricky ordeal whenever you have to give up the default settings.
%%% 非常棘手设置这个内存
%%% This module
...
%%% 能判断这个是不是个问题，但是只能提供一小部分的帮助去知道是什么问题
%%% 
...
%%%    are two kinds of carriers: multiblock(多块) and single block(一块). The default
...
%%%
%%%    There is however a threshold above which an item in memory won't fit a multiblock carrier.
%%%    When that happens, the specific allocator does
