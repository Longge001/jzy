...
handle_call({inform_chat, IdA, IdB}, _From, State) ->
	...
	case get(UidKey) of
		undefined ->
			put(UidKey,[IdA]),
			Msg = lists:concat(["auto sys count:1"]),
			lib_log_api:log_talk_report(IdA, IdB, Msg, utime:unixtime()),
			Reply=1;
		Value ->
				...
				false ->
					...
					case Count>?ALLOW_INFORM_NUM of
						...
						false ->       
							NewValue = [IdA|Value],
							put(UidKey,NewValue),
							Msg = lists:concat(["auto sys count:", length(NewValue)]),
							lib_log_api:log_talk_report(IdA, IdB, Msg, utime:unixtime()),
							Reply = length(NewValue)
					end
			end
...
handle_call(Event, _From, Status) ->
    catch ?ERR("mod_chat_forbid:handle_call not match: ~p~n", [Event]),
    {reply, ok, Status}.
...
handle_cast(Event, Status) ->
    catch ?ERR("mod_chat_forbid:handle_cast not match: ~p~n", [Event]),
    {noreply, Status}.
