...
%	% ?MYLOG("holy", "ZoneMap ~p~n", [ZoneMap]),
	...
	#holy_spirit_battle_state{zone_map = ZoneMap, ref = Ref}.

async_init(State, InfoList) ->
	ZoneMap = info_list2zone_map(InfoList),
	Ref = lib_holy_spirit_battlefield:get_open_ref([]),
	%%推送每个服推送下
	spawn(fun() ->
		F1 = fun(_ZoneId, Group, _AccList) ->
			case Group of
				#server_group{server_list = ServerList} ->
					[
						mod_clusters_center:apply_cast(ServerId, mod_global_counter, set_count, [?MOD_HOLY_SPIRIT_BATTLEFIELD, 1, ModId])
						|| #server_msg{server_id = ServerId, mod_id = ModId} <- ServerList
					];
				_ ->
					skip
			end
			 end,
		maps:fold(F1, [], ZoneMap)
	end),
	State#holy_spirit_battle_state{zone_map = ZoneMap, ref = Ref, is_init = 1}.
...
sync_zone_group(State, InfoList) ->
	...
	State#holy_spirit_battle_state{zone_map = ZoneMap}.

info_list2zone_map(InfoList) ->
	F = fun({ZoneId, {ZoneBases, GroupInfo}}, AccMap) ->
		Servers = [zone_base2server_msg(ZoneBase)||ZoneBase<-ZoneBases],
		#zone_group_info{group_mod_servers = GroupModServers} = GroupInfo,
		BattlefieldList = [group_mod_data2battlefield_msg(ModGroupData, Servers, ZoneId)||ModGroupData<-GroupModServers],
		[#zone_mod_group_data{mod = MaxMod}|_] = lists:reverse(lists:keysort(#zone_mod_group_data.mod, GroupModServers)),
		Group = #server_group{
			zone_id = ZoneId, server_list = Servers, battlefield_list = BattlefieldList,
			max_mod = MaxMod, num = length(ZoneBases)
		},
		AccMap#{ZoneId => Group}
		end,
	lists:foldl(F, #{}, InfoList).

zone_base2server_msg(ZoneBase) ->
	#zone_base{zone = ZoneId, server_id = ServerId, time = Time, merge_ids = MergeIds, server_num = ServerNum
		, server_name = ServerName, world_lv = WorldLv} = ZoneBase,
	#server_msg{
		zone_id = ZoneId,
		server_id = ServerId,
		time = Time,
...
group_mod_data2battlefield_msg(ModGroupData, Servers, ZoneId) ->
	#zone_mod_group_data{group_id = GroupId, mod = Mod, server_ids = ServerIds} = ModGroupData,
	List = [Server#server_msg{mod_id = Mod, battlefield_id = GroupId}||
		Server<-Servers, lists:member(Server#server_msg.server_id, ServerIds)],
	#battlefield_msg{server_list = List, battlefield_id = GroupId, zone_id = ZoneId}.
...
re_alloc_group(_State) ->
	...
	spawn(fun() ->
		F1 = fun(_ZoneId, Group, _AccList) ->
			case Group of
				#server_group{server_list = ServerList} ->
%%							mod_global_counter:set_count(?MOD_HOLY_SPIRIT_BATTLEFIELD, 1, ModId),
					[
						begin
							?PRINT("modid ~p~n", [ModId]),
							mod_clusters_center:apply_cast(ServerId, mod_global_counter, set_count, [?MOD_HOLY_SPIRIT_BATTLEFIELD, 1, ModId])
						end
						|| #server_msg{server_id = ServerId, mod_id = ModId} <- ServerList];
				_ ->
...
%%	?PRINT("send_all_act_open ++++++++++++++++++++++~n", []),
	mod_clusters_center:apply_to_all_node(lib_holy_spirit_battlefield, send_all_act_open, []),
	mod_clusters_center:apply_to_all_node(lib_holy_spirit_battlefield, send_open_TV, []),
%%	mod_clusters_center:apply_to_all_node(lib_chat, send_TV, [{all}, ?MOD_HOLY_SPIRIT_BATTLEFIELD, 1, []]),
	State#holy_spirit_battle_state{ref = NewRef, end_time = utime:unixtime() + WaitTime, status = ?wait, exp_map = #{}, exp_ref = ExpRef}.
...
%%	?MYLOG("holy", "act_end +++++++++++++++++++++++++++++++~n", []),
	...
	State#holy_spirit_battle_state{end_time = 0, status = ?close, exp_map = #{},
		max_copy_id = 0, ref = [], zone_map = NewZoneMap}.
...
get_info(ServerId, RoleId, State) ->
	...
	% 默认1服模式
	{ok, DefaultBin} = pt_218:write(21801, [1, Status, EndTime, []]),
...
%%							lib_activitycalen_api:role_success_end_activity(RoleId, ?MOD_HOLY_SPIRIT_BATTLEFIELD, 0),
							...
							lib_holy_spirit_battlefield:send_to_client(ServerId, RoleId, Bin),
							mod_clusters_center:apply_cast(ServerId, lib_activitycalen_api, role_success_end_activity,
								[RoleId, ?MOD_HOLY_SPIRIT_BATTLEFIELD, 0]),
                            % 事件触发
                            CallbackData = #callback_join_act{type = ?MOD_HOLY_SPIRIT_BATTLEFIELD},
                            lib_holy_spirit_battlefield:apply_cast(ServerId, lib_player_event, async_dispatch, [RoleId, ?EVENT_JOIN_ACT, CallbackData]),
...
%%	lib_player:apply_cast(RoleId, ?APPLY_CAST_STATUS, lib_holy_spirit_battlefield, add_exp, []),
	[[[mod_clusters_center:apply_cast(ServerId, lib_player, apply_cast, [RoleId, ?APPLY_CAST_STATUS, lib_holy_spirit_battlefield, add_exp, []])
		|| #role_msg{server_id = ServerId, role_id = RoleId} <- RoleList]
...
center_connected(_ServerId, _OpTime, _WorldLv, _ServerNum, _ServerName, _MergeSerIds, State) ->
	#holy_spirit_battle_state{status = Status, end_time = EndTime, calc_ref = OldCalcRef, zone_map = ZoneMap} = State,
	if
		Status =/= ?close ->
