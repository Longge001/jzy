...
-module(mod_holy_spirit_battlefield_local).
-author("carlos").
...
start_link() ->
	gen_server:start_link({local, mod_holy_spirit_battlefield_local}, mod_holy_spirit_battlefield_local, [], []).
...
add_exp(RoleId, ExpAdd) ->
	gen_server:cast(?MODULE, {add_exp, config:get_server_id(), RoleId, ExpAdd}).
...
init([]) ->
	State = lib_holy_spirit_battlefield_mod_local:init(),
	{ok, State}.
...
do_handle_cast({revive, ServerId, RoleId}, State) ->
	case lib_holy_spirit_battlefield_mod_local:get_pk_pid(ServerId, RoleId, State) of
		Pid when is_pid(Pid) ->
%%			?MYLOG("holy", "revive ++++++++++++++++++++++  ~n", []),
...
%%	##battle_return_atter{id = AtterId} = Atter,
	lib_holy_spirit_battlefield_mod_local:kill_enemy(SceneId, PoolId, RoomId, DefRoleId, Atter, HitList, State),
	{noreply, State};
...
do_handle_cast({quit_scene, ServerId, RoleId}, State) ->
	NewState = lib_holy_spirit_battlefield_mod_local:quit_scene(ServerId, RoleId, State),
	{noreply, NewState};

do_handle_cast({enter_scene, Turn, Career, PictureId, Picture, Lv, ServerNum, ServerId, RoleId, Power, RoleName}, State) ->
	NewState = lib_holy_spirit_battlefield_mod_local:enter_scene(Turn, Career, PictureId, Picture, Lv, ServerNum, ServerId, RoleId, Power, RoleName, State),
	{noreply, NewState};

do_handle_cast({get_info, ServerId, RoleId}, State) ->
	lib_holy_spirit_battlefield_mod_local:get_info(ServerId, RoleId, State),
	{noreply, State};
...
do_handle_cast({gm_act_start}, State) ->
	?PRINT("gm_act_start ++++++++++++++++++++ ~n", []),
	#holy_spirit_battle_state{end_time = EndTime} = State, 
	case utime:unixtime() < EndTime of 
		true -> 
			NewState = State;
		_ ->
			NewState = lib_holy_spirit_battlefield_mod_local:act_open(State)
	end, 
	{noreply, NewState};
...
%%	?MYLOG("holy2", " ++++++++++++~n", []),
	NewState = lib_holy_spirit_battlefield_mod_local:gm_end(State),
	{noreply, NewState};

do_handle_cast({gm_pk_start}, State) ->
	NewState = lib_holy_spirit_battlefield_mod_local:start_pk(State),
	{noreply, NewState};
...
%%	?MYLOG("holy", "add exp ~n", []),
	NewState = lib_holy_spirit_battlefield_mod_local:add_exp(State),
	{noreply, NewState};
...
do_handle_info({act_end}, State) ->
	NewState = lib_holy_spirit_battlefield_mod_local:act_end(State),
	{noreply, NewState};
