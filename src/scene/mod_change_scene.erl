...
%% 排队切换场景报名
handle_info({'change_scene', PlayerId, Pid, SceneId, ScenePoolId, CopyId, X, Y, NeedOut, Value}, _StateName, State) ->
    NewState = case lists:keyfind(PlayerId, 1, State) of
        false -> State++[{PlayerId, Pid, SceneId, ScenePoolId, CopyId, X, Y, NeedOut, Value}];
        _ -> State
    end,
    {next_state, heartbeat, NewState};
...
manger([{_PlayerId, Pid, SceneId, ScenePoolId, CopyId, X, Y, NeedOut, Value} | T], Pre) ->
    catch gen_server:cast(Pid, {'change_scene', SceneId, ScenePoolId, CopyId, X, Y, NeedOut, Value}),
    manger(T, Pre+1).
