%%----------------------------------------------------
%% Erlang模块热更新到所有线路（包括server的回调函数，如果对state有影响时慎用）
%%
%% 检查：u:c()                %% 列出前5分钟内编译过的文件
%%      u:c(N)               %% 列出前N分钟内编译过的文件
%%
%% 更新：u:u()                %% 更新前5分钟内编译过的文件
%%      u:u(N)               %% 更新前N分钟内编译过的文件
%%      u:u([mod_xx, ...])   %% 指定模块（不带后缀名）
%%      u:u(m)               %% 编译并加载文件
%%
%% Tips: u - update, c - check
%%----------------------------------------------------
-module(u).
-compile(export_all).
-include_lib("kernel/include/file.hrl").
-include("record.hrl").
-include("def_cache.hrl").
-include("common.hrl").

c() ->
    c(5).
c(S) when is_integer(S) ->
    case file:list_dir(".") of
        {ok, FileList} ->
            Files = get_new_file(FileList, S * 60),
            info("---------check modules---------~n~w~n=========check modules=========", [Files]);
        Any ->
            info("Error Dir: ~w", [Any])
    end;
c(_) ->
    info("ERROR======> Badarg", []).

admin()->
    spawn(fun()->u(m) end),
    ok.

u() ->
    u(5).
u(m) ->
    StartTime = utime:unixtime(),
    info("----------makes----------", []),
    c:cd("../"),
    Files = make:all(),
    c:cd("ebin"),
    EndTime = utime:unixtime(),
    Time = EndTime - StartTime,
    info("Recompile: ~s\n",[Files]),
    info("Make Time : ~w s", [Time]),
    u(Time / 60);
u(S) when is_number(S) ->
    c:cd("../ebin"),
    case file:list_dir(".") of
        {ok, FileList} ->
            Files = get_new_file(FileList, util:ceil(S * 60) + 3),
            info("---------modules---------~n~w~n----------nodes----------", [Files]),
            %% AllZone = mod_disperse:node_all_list(),
            %% loads(AllZone, Files);
            load(Files);
        Any ->
            info("Error Dir: ~w", [Any])
    end;
u(Files) when is_list(Files) ->
    info("---------modules---------~n~w~n----------nodes----------", [Files]),
    %% AllZone = mod_disperse:node_all_list(),
    %% loads(AllZone, Files);
    load(Files);
u(_) ->
    info("ERROR======> Badarg", []).

%% 更新到所有节点: 只再开发环境下使用，运营环境更新到当前节点
uu(Files) when is_list(Files) ->
    info("---------modules---------~n~w~n----------nodes----------", [Files]),
    load(Files),
    AllNodeList = mod_disperse:node_all_list(),
    SelfNode = node(),
    OtherNodeList = [Node || #node{node = NodeName} = Node <- AllNodeList,  NodeName =/= SelfNode],
    loads(OtherNodeList, Files);
uu(_) ->
    info("ERROR======> Badarg", []).

%% m(['src/data/*','src/lib/lib_goods.erl'])
m(Files) when is_list(Files) ->
    StartTime = utime:unixtime(),
    info("----------makes----------~n~w~n", [Files]),
    c:cd("../"),
    Res = make:files(Files, [debug_info,{i, "include"},{outdir, "ebin"}]),
    c:cd("ebin"),
    EndTime = utime:unixtime(),
    Time = EndTime - StartTime,
    info("Make Time : ~w s", [Time]),
    Res.

info(V) ->
    info(V, []).
info(V, P) ->
    io:format(V ++ "~n", P).

%% 更新到所有线路
loads([], _Files) -> ok;
loads([H | T], Files) ->
    info("[~w]", [H#node.node]),
    rpc:cast(H#node.node, u, load, [Files]),
    loads(T, Files).

get_new_file(Files, S) ->
    get_new_file(Files, S, []).
get_new_file([], _S, Result) -> Result;
get_new_file([H | T], S, Result) ->
    NewResult
        = case string:tokens(H, ".") of
            [Left, Right] when Right =:= "beam" ->
                case file:read_file_info(H) of
                    {ok, FileInfo} ->
                        Now = calendar:local_time(),
                        case calendar:time_difference(FileInfo#file_info.mtime, Now) of
                            {Days, Times} ->
                                Seconds = calendar:time_to_seconds(Times),
                                case Days =:= 0 andalso Seconds < S of
                                    true ->
                                        FileName = list_to_atom(Left),
                                        [FileName | Result];
                                    false -> Result
                                end;
                            _ -> Result
                        end;
                    _ -> Result
                end;
            _ -> Result
        end,
    get_new_file(T, S, NewResult).

load(Files) ->
    NowTime = utime:unixtime(),
    ok = do_load(Files),
    ok = lib_log_api:log_server_hot(Files, NowTime, utime:unixtime()),
    ok.

do_load([]) -> ok;
do_load([FileName | T]) ->
    c:l(FileName),
    catch lib_u:after_load_event(FileName),
    info("loaded: ~w", [FileName]),
    do_load(T).
%% case code:soft_purge(FileName) of
%%     true ->
%%         case code:load_file(FileName) of
%%             {module, _} ->
%%                 info("loaded: ~w", [FileName]),
%%                 ok;
%%                 %% info("loaded: ~w", [FileName]);
%%             {error, What} -> info("ERROR======> loading: ~w (~w)", [FileName, What])
%%         end;
%%     false -> info("ERROR======> Processes lingering : ~w [zone ~w] ", [FileName, srv_kernel:zone_id()])
%% end,
%% load(T).
