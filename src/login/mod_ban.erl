...
check(Id, Ip) ->
	case misc:whereis_name(global, ?MODULE) of
		Pid when is_pid(Pid) ->
			gen_server:call(Pid, {check, [Id, Ip]}, 2000);
		_r ->
			%% 进程不存在,直接允许玩家登陆
			passed
	end.
...
handle_call({check, [Id, Ip]}, _From, State) ->
    {Reply, NewState} = private_check(Id, Ip, State),
        ...
        true -> %% 登陆成功记录下时间
            ...
            case State#rc_ban.ban_all =:= true of
                ...
                false ->
					case lists:member(Ip, BaiIpList) of
						true ->
                            NewPlayerIpDict = dict:store(Id, tool:ip2bin(Ip), State#rc_ban.player_ip_dict),
...
private_unbai_ip(IpR, State) ->
    ...
    NewIpList = State#rc_ban.bai_list -- IpL,
	{ok, State#rc_ban{bai_list = NewIpList}}.
...
private_ban_ip(IpR, State) ->
    ...
    IpList = [IpR],
    #rc_ban{ban_ip_list = BanIpList} = State,
    F = fun(Ip) ->
        HasBanned = lists:member(Ip, BanIpList),
        if
            Ip == 0; HasBanned -> skip;
            true -> erlang:spawn(fun() -> kick_out_player(Ip, State) end)
        end
    end,
    ...
    AddBanIpList = [Ip || Ip <- IpList, not lists:member(Ip, BanIpList)],
    {ok, State#rc_ban{ban_ip_list = AddBanIpList ++ BanIpList}}.
...
private_unban_ip(IpR, State) ->
    ...
    NewBanIpList = State#rc_ban.ban_ip_list -- IpList,
...
kick_out_player(Ip, State) ->
    ...
    IpBin = tool:ip2bin(Ip),
    dict:map(fun(Id, Value) ->
                case Value == IpBin of
                    true ->
...
db_ban_login_ip(Ip, Type) ->
	Data = [Type, tool:ip2bin(Ip)],
	SQL  = io_lib:format("update player_login set status = ~p where last_login_ip = '~s'", Data),
    db:execute(SQL).
