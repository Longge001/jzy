...
login_reward_login(Ps) ->
    ...
    ReSql = io_lib:format(?SelectLoginRewardPlayerSql, [RoleId]),
    ...
    lib_hi_point_api:hi_point_task_login(RoleId, RoleLv),
    %%更新数据库
    save_to_db(NewLoginReward, RoleId),
    %%推送客户端
    #login_reward{login_day = _Day} = NewLoginReward,
    %%    _LoginDay = lib_login_reward:get_login_day(_Day, RoleId),
    ?PRINT("17500  Day, NewRewardStatus  :~p  , ~p~n", [_Day, NewRewardStatus]),
    {ok, Bin} = pt_175:write(17500, [?SUCCESS, _Day, NewRewardStatus]),
    lib_server_send:send_to_sid(Sid, Bin),
    NewPs = Ps#player_status{
        login_reward = NewLoginReward
    },
    NewPs.
...
across_zero_helper(Ps) ->
    ...
    %%    ?DEBUG("zero ~n", []),
    %%推送客户端
    {ok, Bin} = pt_175:write(17500, [?SUCCESS, LoginDay, NewRewardStatus]),
    lib_server_send:send_to_sid(Sid, Bin),
    NewPs = Ps#player_status{
        login_reward = NewLoginReward
    },
...
across_zero(#player_status{id = RoleId}) ->
    %%    ?DEBUG("zero ~n", []),
    lib_player:apply_cast(RoleId, ?APPLY_CAST_SAVE, lib_login_reward, across_zero_helper, []).
...
init_reward_status() ->
    ...
    DayList = data_login_reward:get_login_reward_day_list(),
...
get_login_day(Day, LastLoginTime, _RoleId) ->
    %%	%%上一次登录时间
    %%	LastLoginTime = case lib_role:get_role_show(RoleId) of
    %%		#ets_role_show{last_login_time = _Time} ->
    %%			?MYLOG("cym", "lastTime ~p~n now ~p~n",  [utime:unixdate(_Time), utime:unixdate()]),
    %%			_Time;
    %%		_ ->
    %%			utime:unixdate() - 86400 + 1
    %%	end,
...
update_login_reward_status(LoginDay, RewardStatus) ->
    ...
    %%    F = fun(Reward, AccList) ->
    %%        {Day, Status} = Reward,
    %%        NewReward = case Status of %%本来状态
    %%                        0 -> %%
    %%                            if
    %%                                LoginDay >= Day -> %%如果达到了次数
    %%                                    {Day, 1};      %%可以领取
    %%                                true ->
    %%                                    {Day, Status} %%还不能领取
    %%                            end;
    ...
    %%        [NewReward | AccList]
    %%        end,
    %%    NewRewardStatus = lists:reverse(lists:foldl(F, [], RewardStatus)),
    NewRewardStatus.
...
save_to_db(LoginReward, RoleId) ->
    ...
    ReSql = io_lib:format(?ReplaceLoginRewardPlayerSql, [RoleId, util:term_to_string(RewardList), CreateTime, Day, Time]),
    %%    ?DEBUG("~s~n", [ReSql]),
    db:execute(ReSql).

%%重置领取状态
reset(#player_status{id = ROleId} = Status, Day) ->
    #player_status{login_reward = LoginReward} = Status,
    %%	#login_reward{create_time = _CreateTime} =  LoginReward,
    %%    NewCreateTime = utime:unixtime() - (Day - 1) * 86400,
    %%	LoginDay = get_login_day(NewCreateTime),
    NewLoginReward = LoginReward#login_reward{reward_list = update_login_reward_status(Day, init_reward_status()), login_day = Day},
    save_to_db(NewLoginReward, ROleId),
    {ok, Status#player_status{login_reward = NewLoginReward}}.

%%获取奖励
get_reward_by_sex(Sex, DayId) ->
    case data_login_reward:get_login_reward_day_con(DayId) of
        #login_reward_day_con{reward_list = _Reward} ->
...
%%获取合服奖励
get_merge_reward_by_sex(Sex, DayId) ->
    case data_login_reward:get_login_reward_day_con(DayId) of
        #login_reward_day_con{reward_list = _Reward} ->
            get_reward_by_sex_helper(_Reward, Sex);
        _ ->
